...
import kss
import logging
# from PIL import Image # PIL Image는 현재 코드에서 사용되지 않아 주석 처리
...
def load_model():
    ...
    model = SentenceTransformer("jhgan/ko-sbert-nli") # "jhgan/ko-sbert-nli" 대신 "snunlp/KR-SBERT-V40K-KoNLI-STS" 또는 최신 한국어 모델 사용 고려
...
def calculate_text_lines(text, max_chars_per_line):
    lines = 0
    if not text: # 텍스트가 비어있으면 0줄 반환
        return 0
    for paragraph in text.split('\n'):
        # 각 줄을 max_chars_per_line에 맞춰 감싸고, 그 결과의 줄 수를 계산
        wrapped_lines = textwrap.wrap(paragraph, width=max_chars_per_line, break_long_words=True, replace_whitespace=False)
        lines += len(wrapped_lines) if wrapped_lines else 1 # 빈 줄도 1줄로 간주될 수 있으므로, 내용이 있을 때만 len, 아니면 0 또는 1 (상황에 따라)
    return lines if lines > 0 else 1 # 최소 1줄로 처리 (내용이 조금이라도 있다면)

def smart_sentence_split(text):
    try:
        sentences = kss.split_sentences(text)
        # 3번 요구사항 반영 시도: 매우 짧은 "문장" (예: 단어)은 그대로 두거나, 특정 기준으로 필터링
        # 여기서는 kss 결과를 우선적으로 사용
        return sentences
    except Exception: # kss 실패 시 기본 분리
        return [s.strip() for s in re.split(r'[.!?]\s+', text) if s.strip()]

def is_potentially_non_sentence(sentence_text, min_length=5):
    """
    요구사항 3: 문장이 아닌 것을 식별하기 위한 간단한 휴리스틱.
    매우 짧고, 일반적인 문장 부호로 끝나지 않는 경우.
    """
    sentence_text = sentence_text.strip()
    if not sentence_text:
        return False # 빈 문자열은 처리 대상 아님
    if len(sentence_text) < min_length and not sentence_text.endswith(('.', '!', '?', '다', '요', '죠', '까', '나', '시오')):
        # 예: "항목 1", "소제목" 등
        return True
    # 특정 키워드나 패턴으로 시작하거나 끝나는 경우도 추가 가능
    # 예: if sentence_text.startswith("Q.") or sentence_text.startswith("A."): return True
    return False


def is_incomplete(sentence):
    sentence = sentence.strip()
    if not sentence: return False # 빈 문장은 불완전하지 않음
    # 문장 길이 기반 불완전성 판단 제거 (짧은 문장도 완전할 수 있음)
    ...
    # 접속사로 시작하는 경우 (짧은 문장과 결합될 때 불완전해 보일 수 있음)
    # if re.match(r'^(그리고|하지만|그러나|또한|그래서|즉|또|그러면|그런데)$', sentence) and len(sentence) < 10 :
    # return True

    # 문장 종결 부호가 없고, 특정 길이가 되지 않는 경우 (기존 로직 유지하되, 길이 조건은 완화 또는 다른 기준과 결합)
    # if not sentence.endswith(('.', '!', '?', '다', '요', '죠', '까', '나', '시오')) and len(sentence) < 15:
    # return True
    return False


def merge_sentences(sentences, max_chars_per_sentence_segment=200): # 문장 병합 시 최대 길이 제한 추가
    merged = []
    buffer = ""
    for i, sentence in enumerate(sentences):
        ...
        # 요구사항 3번 반영 시도: "문장이 아닌 것"으로 판단되면 바로 추가 (병합 X)
        if is_potentially_non_sentence(sentence):
            if buffer: # 버퍼에 내용이 있으면 먼저 추가
        ...
        if buffer:
            ...
            if len(current_candidate) > max_chars_per_sentence_segment: # 병합 시 너무 길어지면 분리
                merged.append(buffer)
                buffer = sentence
            elif not is_incomplete(buffer) and is_incomplete(sentence) and i < len(sentences) -1 : # 버퍼는 완전한데, 현재 문장이 불완전하면 일단 합쳐봄
                 buffer = current_candidate
            elif not is_incomplete(buffer): # 버퍼가 이미 완전하면, 현재 문장과 합치지 않고 버퍼를 먼저 추가
                merged.append(buffer)
                buffer = sentence
            else: # 버퍼가 불완전하면 현재 문장과 합침
                buffer = current_candidate

            # 버퍼가 완전해졌거나 마지막 문장이면 merged에 추가
            if not is_incomplete(buffer) or i == len(sentences) - 1:
                merged.append(buffer)
                buffer = ""
        else: # 버퍼가 비어있을 때
            if is_incomplete(sentence) and i < len(sentences) - 1: # 현재 문장이 불완전하고 다음 문장이 있다면 버퍼에 저장
                buffer = sentence
            else: # 현재 문장이 완전하거나 마지막 문장이면 바로 추가
                merged.append(sentence)
                buffer = "" # 버퍼 초기화

    if buffer: # 루프 후 버퍼에 남은 내용 추가
        merged.append(buffer)
    return merged
